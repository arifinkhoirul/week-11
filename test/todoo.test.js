...
const todoRoutes = require("../src/routes/todo.js"); // mengganti path yang sudah sesuai
...
describe("Todo API", () => {
  ...
  it("should retrieve a specific todo", async () => {
    const response = await request(app).get("/todos/1"); // Di ganti dengan ID yang ada

    expect(response.statusCode).toBe(404);
    expect(response.body.id).toBe(undefined); // Menyesuaikan dengan ID yang diharapkan
  });

  it("should update a todo", async () => {
    const response = await request(app)
      .put("/todo/1") // Diganti dengan ID yang ada
      .send({
        title: "undefined",
        durasi: 90,
  ...
  it("should delete a todo", async () => {
    const response = await request(app).delete("/todos/1"); // Digaanti dengan ID yang ada
